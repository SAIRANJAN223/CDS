CLASS LHC_ZI_SERROR_CODE_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE DatabaseForErrorAll\_DatabaseForErrorCod
          ENTITIES_UPDATE FOR UPDATE DatabaseForErrorCod.
ENDCLASS.

CLASS LHC_ZI_SERROR_CODE_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE ZI_SERROR_CODE\_DatabaseForErroText,
          text_for_existing_entity TYPE TABLE FOR CREATE ZI_SERROR_CODE\_DatabaseForErroText,
          text_update              TYPE TABLE FOR UPDATE ZI_SERROR_DESC.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK ZI_SERROR_CODE\_DatabaseForErroText,
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF ZI_SERROR_CODE_S
      ENTITY DatabaseForErrorCod
        CREATE BY \_DatabaseForErroText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF ZI_SERROR_CODE_S
        ENTITY DatabaseForErrorCod BY \_DatabaseForErroText
